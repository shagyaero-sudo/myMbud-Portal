      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-hidden">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl max-h-[85vh] sm:max-h-[80vh] bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden z-10"
          >
            {/* Header Modal */}
            <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between shrink-0">
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-all shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* Score Banner (Horizontal Layout di Layar Lebar) */}
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-2.5 custom-scrollbar">
              {courses.map((course) => (
                <div
                  key={course.id}
                  className="p-3 sm:p-3.5 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-800/80 flex items-center justify-between gap-3"
                >
                  {isEditing ? (
                    <div className="flex-1 flex items-center gap-2.5 min-w-0">
                      <input
                        ...
                        className="flex-1 min-w-0 text-xs sm:text-sm font-semibold px-3 py-1.5 rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 focus:outline-none"
                      />
                      <input
                        type="number"
                        min={1}
                        ...
                        className="w-12 text-xs sm:text-sm text-center font-bold px-1.5 py-1.5 rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 focus:outline-none shrink-0"
                      />
                      <button
                    ...
                    <>
                      ...
                      <select
                        ...
                        className="px-3 py-1.5 rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 text-xs sm:text-sm font-bold text-slate-800 dark:text-zinc-200 focus:outline-none cursor-pointer shrink-0"
                      >
            ...
            {/* Modal Footer */}
            <div className="p-3.5 sm:p-4 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-zinc-900/50">
              {isEditing ? (
                <button
