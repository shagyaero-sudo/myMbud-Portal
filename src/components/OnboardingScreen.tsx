...
>>>>>>> 32f71d6de7829706cb34ff5426c02f5a8b9967b9
    ...
    <div className="fixed inset-0 z-[99999] bg-[#0a0a0a] text-slate-100 flex flex-col font-sans overflow-hidden">
      ...
      <div className="relative z-20 flex justify-end p-6 sm:p-8">
        {currentSlide < slides.length - 1 && (
          <motion.button
            ...
            className="text-xs font-bold tracking-widest text-slate-400 hover:text-white uppercase transition-colors px-4 py-2 rounded-full border border-white/5 hover:bg-white/10 backdrop-blur-md"
          >
            Skip
          </motion.button>
        )}
      ...
      <div className="relative z-20 px-6 pb-12 sm:pb-16 w-full max-w-md mx-auto flex flex-col items-center gap-8">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={handleNext}
          className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 text-sm font-bold text-white shadow-xl ${
            currentSlide === slides.length - 1 
...
export const OnboardingScreen: React.FC<OnboardingScreenProps> = ({
  ...
  const slides = [
    {
      id: 1,
      title: 'Semua Info, Satu Pintu',
      desc: 'Mading pengumuman, kalender interaktif, sampai fast-track presensi kuliah hari ini. Semuanya rapi di satu tempat.',
      icon: CalendarDays,
      gradient: 'from-cyan-400 to-blue-600',
      shadow: 'shadow-blue-500/50',
      glow: 'bg-blue-600/20',
    ...
    {
      id: 2,
      title: 'Tracker Tugas Anti-Kelewat',
      desc: 'Tugas numpuk? Tenang. Pantau deadline, kerjain, lalu Tandai Selesai. Tugas akan otomatis pindah ke riwayat!',
      icon: CheckCircle2,
      gradient: 'from-emerald-400 to-teal-500',
      shadow: 'shadow-emerald-500/50',
      glow: 'bg-emerald-500/20',
    ...
    {
      id: 3,
      title: 'Tools Sakti Mahasiswa',
      desc: 'Mulai dari bikin surat Turlap otomatis, hitung prediksi IP, sampai main minigame Blockblast buat healing sejenak.',
      icon: Blocks,
      gradient: 'from-fuchsia-500 to-purple-600',
      shadow: 'shadow-purple-500/50',
      glow: 'bg-purple-600/20',
    ...
    {
      id: 4,
      title: 'Chat Dosen Tanpa Saltik',
      desc: 'Buku telepon khusus matkulmu. Gunakan fitur Template Chat pintar biar pesan ke Dosen atau PJ selalu rapi dan sopan.',
      icon: MessageSquareText,
      gradient: 'from-amber-400 to-orange-500',
      shadow: 'shadow-amber-500/50',
      glow: 'bg-amber-500/20',
    ...
    {
      id: 5,
      title: 'Bank Materi PDF',
      desc: 'H-1 ETS belum belajar? Buka, baca, beres! Akses arsip PPT dan materi kuliah langsung di dalam portal tanpa perlu download.',
      icon: FileText,
      gradient: 'from-green-400 to-emerald-600',
      shadow: 'shadow-green-500/50',
      glow: 'bg-green-600/20',
    ...
    },
    {
      id: 6,
      title: 'Jangan Ketinggalan Info!',
      desc: 'Dapatkan notifikasi real-time tentang tugas dan pengumuman mading di HP-mu, meskipun myMbud sedang tidak dibuka.',
      icon: BellRing,
      gradient: 'from-rose-400 to-red-600',
      shadow: 'shadow-rose-500/50',
      glow: 'bg-rose-600/20',
  ...
  const finishOnboarding = () => {
    localStorage.setItem('mymbud_onboarded', 'true');
    // Jika mau dipasang fungsi trigger OneSignal permission di sini juga bisa
            ...
            <div className="space-y-4">
              {currentSlide === 0 && (
                <motion.div 
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="inline-block mb-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm text-[10px] uppercase tracking-widest text-slate-300 font-bold"
                >
                  Halo, {userName}! 👋
                </motion.div>
              )}
