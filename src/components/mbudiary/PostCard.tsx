...
export const PostCard: React.FC<PostCardProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (repostMenuRef.current && !repostMenuRef.current.contains(event.target as Node)) setIsRepostMenuOpen(false);
    };
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && selectedImage) setSelectedImage(null);
    ...
    if (selectedImage || isQuoteOpen) document.addEventListener('keydown', handleKeyDown);
  ...
  const handleCommentClick = () => {
    if (onSelectPost && !isDetailPage) onSelectPost(post.id);
    else setIsRepliesExpanded((prev) => !prev);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div onClick={() => onSelectAuthor?.(displayAuthorNrp)} className="flex items-center gap-3 cursor-pointer group/author" title={`Lihat profil ${displayAuthorName}`}>
            ...
            <div>
              ...
              <div className="text-slate-400 dark:text-zinc-500 text-[11px] mt-0.5">@{displayAuthorUsername || 'unknown'}</div>
          ...
          {canDelete && (
            <div ref={menuRef} className="relative shrink-0">
              <button type="button" onClick={() => setIsMenuOpen((prev) => !prev)} disabled={isDeleting} className="p-1.5 rounded-xl text-slate-400 dark:text-zinc-500 hover:text-slate-700 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
                <MoreVertical className="w-5 h-5" />
              </button>
              <AnimatePresence>
                {isMenuOpen && (
                  <motion.div initial={{ opacity: 0, y: -4, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.97 }} className="absolute right-0 top-full mt-1.5 z-30 w-44 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl p-1.5 shadow-xl">
                    <button type="button" onClick={handleDeletePost} disabled={isDeleting} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors disabled:opacity-50">
                      <Trash2 className="w-3.5 h-3.5" /><span>Hapus Postingan</span>
                    </button>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        {isQuoteRepost && <div className="text-[13px] sm:text-sm text-slate-800 dark:text-zinc-200 leading-relaxed whitespace-pre-line mb-4 font-normal">{post.quoteContent}</div>}
        {!isQuoteRepost && displayContent && <div className="text-[13px] sm:text-sm text-slate-800 dark:text-zinc-200 leading-relaxed whitespace-pre-line mb-4 font-normal">{displayContent}</div>}

        {isQuoteRepost && originalPost && (
          <div onClick={() => onSelectPost?.(originalPost.id)} className="mb-4 rounded-2xl border border-slate-200 dark:border-zinc-700 overflow-hidden bg-slate-50/50 dark:bg-zinc-800/40 cursor-pointer hover:bg-slate-100/50 dark:hover:bg-zinc-800 transition-colors w-full">
            ...
            {originalPost.content && <div className="px-4 pb-3 text-[13px] text-slate-700 dark:text-zinc-300 leading-relaxed whitespace-pre-line">{originalPost.content}</div>}
        ...
        {post.isRepost && !originalPost && (
          <div className="mb-4 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-800 text-xs text-slate-400 dark:text-zinc-500 italic">
          ...
          {isRepliesExpanded && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-4 pt-4 border-t border-slate-100 dark:border-zinc-800 space-y-3 overflow-hidden">
                      ...
                      <div key={reply.id} className="p-3.5 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-800 space-y-1.5 w-full">
                        <div className="flex items-center justify-between text-xs">
                          <div onClick={() => onSelectAuthor?.(reply.authorNrp)} className="flex items-center gap-2 cursor-pointer group/replyAuthor">
                            ...
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className="text-slate-900 dark:text-zinc-100 font-bold text-xs sm:text-[13px] group-hover/replyAuthor:text-indigo-600 dark:group-hover/replyAuthor:text-indigo-400 transition-colors">{replyName}</span>
                        ...
                        <p className="text-[13px] text-slate-700 dark:text-zinc-300 leading-relaxed pl-8">{reply.content}</p>
                      </div>
                    );
                  })
                )}
              ...
              <form onSubmit={handleAddReply} className="flex items-center gap-2 pt-2">
                <input type="text" value={replyContent} onChange={(e) => setReplyContent(e.target.value)} placeholder={`Tulis komen sebagai ${currentUser.nickname}...`} className="flex-1 px-4 py-2.5 rounded-2xl bg-slate-50 dark:bg-zinc-800 text-xs sm:text-[13px] border border-slate-200 dark:border-zinc-800 text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                <button type="submit" disabled={!replyContent.trim() || isSubmittingReply} className="px-4 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-100 dark:disabled:bg-zinc-800 text-white disabled:text-slate-400 text-xs font-semibold transition-all flex items-center gap-1 shadow-sm"><Send className="w-4 h-4" /><span className="hidden xs:inline">Kirim</span></button>
              </form>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <AnimatePresence>
        {isQuoteOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`fixed inset-0 z-[999998] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 transition-all duration-300 ${isQuoteFocused ? 'pb-[26dvh] sm:pb-0' : 'pb-0'}`} onMouseDown={(e) => { if (e.target === e.currentTarget) { setIsQuoteOpen(false); setQuoteContent(''); setIsQuoteFocused(false); } }}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-3xl p-5 sm:p-6 shadow-2xl flex flex-col max-h-[85dvh]" onMouseDown={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4 shrink-0">
                ...
                <button type="button" onClick={() => { setIsQuoteOpen(false); setQuoteContent(''); setIsQuoteFocused(false); }} className="p-2 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <form onSubmit={handleQuoteRepost} className="flex flex-col flex-1 min-h-0">
                <textarea autoFocus value={quoteContent} onChange={(e) => setQuoteContent(e.target.value)} onFocus={() => setIsQuoteFocused(true)} onBlur={() => setIsQuoteFocused(false)} placeholder="Apa pendapatmu tentang postingan ini?" rows={3} className="w-full resize-none px-4 py-3 mb-3 rounded-2xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-sm text-slate-900 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 shrink-0" />
                <div className="rounded-2xl border border-slate-200 dark:border-zinc-700 overflow-y-auto bg-slate-50/50 dark:bg-zinc-800/40 flex-1 min-h-0 mb-3 custom-scrollbar">
                  <div className="px-4 py-3 flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <div className="text-xs sm:text-[13px] font-bold text-slate-900 dark:text-zinc-100 truncate">{post.isRepost && originalPost ? originalAuthorName : authorName}</div>
                      </div>
                      <div className="text-[10px] text-slate-400 dark:text-zinc-500">@{post.isRepost && originalPost ? originalAuthorProfile?.username || 'unknown' : authorProfile?.username || 'unknown'}</div>
                    </div>
                  </div>
                  <div className="px-4 pb-4 text-[13px] text-slate-700 dark:text-zinc-300 leading-relaxed whitespace-pre-line">{post.isRepost && originalPost ? originalPost.content || 'Postingan dengan gambar' : post.content || 'Postingan dengan gambar'}</div>
                </div>
                <div className="flex items-center justify-end gap-2 shrink-0">
                  ...
                  <button type="submit" disabled={!quoteContent.trim() || isReposting} className="px-5 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-100 dark:disabled:bg-zinc-800 text-white disabled:text-slate-400 text-xs font-semibold transition-all flex items-center gap-1.5 shadow-sm"><Quote className="w-4 h-4" /><span>{isReposting ? 'Memproses...' : 'Quote Repost'}</span></button>
                </div>
              </form>
            </motion.div>
          </motion.div>
