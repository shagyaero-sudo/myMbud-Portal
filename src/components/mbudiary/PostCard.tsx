...
export const PostCard: React.FC<PostCardProps> = ({
  ...
  const originalPost =
    post.isRepost && post.originalPostId
      ? getPosts().find((item) => item.id === post.originalPostId)
      : null;
  ...
  const handleLikeToggle = async () => {
    ...
    if (updated) {
      const hasLiked = updated.likes.includes(currentUser.nrp.toLowerCase());
      setIsLiked(hasLiked);
  ...
  return (
    <>
      ...
      >
        {/* =====================================================
            REPOST LABEL (Hanya muncul jika bukan Quote Repost)
            ===================================================== */}
        {isPlainRepost && (
                ...
                {originalPost.imageUrls.map((imageUrl, index) => (
                  <div
                    ...
                    className={`relative overflow-hidden rounded-xl bg-slate-100 dark:bg-zinc-800 ${
                      originalPost.imageUrls?.length === 1 ? 'max-h-[400px]' : 'aspect-square'
                    }`}
                  >
                    <img
                      src={imageUrl}
              ...
              <div className="space-y-2.5">
                {replies.length === 0 ? (
                  ...
                  replies.map((reply) => {
                    ...
                    return (
                      <div key={reply.id} className="p-3 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-800 space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          <div
                            onClick={() => onSelectAuthor?.(reply.authorNrp)}
              ...
              <form onSubmit={handleQuoteRepost} className="flex flex-col flex-1 min-h-0">
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-zinc-700 overflow-y-auto bg-slate-50/50 dark:bg-zinc-800/40 flex-1 min-h-0 mb-3 custom-scrollbar">
                  ...
                  <div className="px-3 pb-3 text-xs text-slate-700 dark:text-zinc-300 leading-relaxed whitespace-pre-line">
                    {post.isRepost && originalPost ? originalPost.content || 'Postingan dengan gambar' : post.content || 'Postingan dengan gambar'}
                  </div>
                </div>
