...
// IMPORT KOMPONEN DAN HELPER MUSIK
                      ...
                      {selectedTrack && (
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="p-1 rounded-full text-rose-500 hover:bg-rose-50 dark:hover:bg-zinc-800 transition-colors"
                            title="Hapus Musik"
                          >
                ...
                <div className="px-4 py-2.5 sm:px-5 border-t border-slate-200/40 dark:border-white/10 bg-white/50 dark:bg-zinc-900/50 flex items-center justify-between shrink-0 relative z-20 rounded-b-3xl">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <ChevronDown className={`w-3 h-3 text-slate-400 transition-transform ${isAudienceDropdownOpen ? 'rotate-180' : ''}`} />
                    </button>

                    <AnimatePresence>
                      {isAudienceDropdownOpen && (
                        <>
                          <div className="fixed inset-0 z-30" onClick={() => setIsAudienceDropdownOpen(false)} />
                          <motion.div
                            initial={{ opacity: 0, y: 5, scale: 0.97 }}
                            animate={{ opacity: 1, y: 0, scale: 1 }}
                            exit={{ opacity: 0, y: 5, scale: 0.97 }}
                            className="absolute right-0 bottom-full mb-2 z-40 w-44 bg-white/95 dark:bg-zinc-800/95 backdrop-blur-xl border border-white/60 dark:border-zinc-700 rounded-2xl p-1.5 shadow-xl space-y-1"
                          >
                            <button
                              type="button"
                              onClick={() => {
                              ...
                              className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-xs font-medium transition-colors cursor-pointer ${
                                !isFollowersOnly
                                  ? 'bg-slate-100 dark:bg-zinc-700/80 text-slate-900 dark:text-zinc-100 font-semibold'
                            ...
                            >
                              ...
                              {!isFollowersOnly && <Check className="w-3.5 h-3.5 text-blue-500" />}
                            </button>

                            <button
                              type="button"
                              onClick={() => {
                                setIsFollowersOnly(true);
                                setIsAudienceDropdownOpen(false);
                              }}
                              className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-xs font-medium transition-colors cursor-pointer ${
                                isFollowersOnly
