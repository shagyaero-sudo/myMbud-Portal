...
export const CreatePostForm: React.FC<CreatePostFormProps> = ({
  ...
  return (
    <>
      {/* COMPACT TRIGGER BAR DI FEED (DIET & RAMPING TANPA BLANK SPACE) */}
      <div className="px-3.5 py-2.5 sm:px-4 sm:py-3 w-full bg-white/40 dark:bg-zinc-950/20 transition-colors">
        <AnimatePresence>
          {showSuccessToast && (
            <motion.div
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <div
            ...
            className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white/60 dark:bg-zinc-800 shrink-0 flex items-center justify-center overflow-hidden border border-slate-200/80 dark:border-zinc-700/80 cursor-pointer hover:opacity-90 transition-opacity"
          >
          ...
          {/* INPUT TRIGGER BUTTON (BUKA MODAL) */}
          <div
            onClick={() => setIsModalOpen(true)}
            className="flex-1 min-w-0 bg-transparent py-1.5 text-xs sm:text-[13px] text-slate-400 dark:text-zinc-500 hover:text-slate-600 dark:hover:text-zinc-300 cursor-pointer transition-colors truncate"
          >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {/* TOMBOL UPLOAD GAMBAR DENGAN KETERANGAN TEKS */}
            <button
              type="button"
              onClick={() => {
                setIsModalOpen(true);
              ...
              className="px-2.5 py-1.5 rounded-xl bg-blue-50/60 hover:bg-blue-100/70 dark:bg-blue-950/40 dark:hover:bg-blue-900/50 text-blue-600 dark:text-blue-400 text-xs font-semibold transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer"
            >
              <ImagePlus className="w-3.5 h-3.5" />
              <span className="hidden xs:inline text-[11px] sm:text-xs">Upload Gambar</span>
            </button>

            {/* TOMBOL POSTING UTAMA */}
            <button
              type="button"
              onClick={() => setIsModalOpen(true)}
              className="px-3 py-1.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-all flex items-center gap-1 shadow-xs active:scale-95 cursor-pointer"
            >
      ...
      {/* POPUP MODAL BUAT POSTINGAN LENGKAP */}
          ...
          {isModalOpen && (
            ...
            >
              ...
              >
                ...
                <div className="p-4 sm:p-5 overflow-y-visible flex-1 min-h-[120px] relative">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0 relative">
                      ...
                      {/* DROPDOWN MENTION TEPAT DI BAWAH KURSOR */}
                ...
                {/* MODAL FOOTER DENGAN OPSI UPLOAD */}
