...
export const TaskTrackerView: React.FC<TaskTrackerViewProps> = ({
  ...
  const [existingAttachment, setExistingAttachment] = useState<{ fileName: string; fileUrl: string } | null>(null);
  ...
  const getDeadlineBadge = (deadlineStr: string) => {
    ...
    if (diffDays < 0) {
      return { label: 'Tenggat Lewat', bg: 'bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 border-slate-200 dark:border-zinc-700' };
    } else if (diffDays <= 2) {
      const dayText = diffDays <= 0 ? 'H-0' : `H-${diffDays}`;
      return { label: `URGENT ${dayText}`, bg: 'bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-900/50 animate-pulse' };
    } else if (diffDays <= 5) {
      return { label: `Mepet H-${diffDays}`, bg: 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-900/50' };
    } else {
      return { label: `Masih H-${diffDays}`, bg: 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900/50' };
    }
  };
      ...
      <div className="space-y-4">
        ...
        {filteredTasks.length === 0 ? (
          <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl p-12 text-center text-slate-400 dark:text-zinc-500 text-xs shadow-sm">
            {activeTab === 'active' ? 'Tidak ada tugas aktif yang sesuai.' : 'Belum ada riwayat tugas yang telah berlalu.'}
          </div>
        ) : activeTab === 'active' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredTasks.map((t) => {
              const badge = getDeadlineBadge(t.deadline);
              const formattedDate = new Date(t.deadline).toLocaleString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });

              return (
                <div key={t.id} onClick={() => setSelectedDetailTask(t)} className="bg-white dark:bg-zinc-900 rounded-3xl p-6 shadow-[0_4px_25px_-5px_rgba(0,0,0,0.03)] dark:shadow-none space-y-4 border border-slate-100 dark:border-zinc-800 hover:shadow-xl hover:-translate-y-1 transition-all duration-200 cursor-pointer flex flex-col justify-between group">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/60 px-3 py-1 rounded-full">{t.course}</span>
                      {badge && <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${badge.bg}`}>{badge.label}</span>}
                    </div>
                    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
             {filteredTasks.map((t) => {
              const formattedDate = new Date(t.deadline).toLocaleString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });
              return (
                <div key={t.id} onClick={() => setSelectedDetailTask(t)} className="bg-white/80 dark:bg-zinc-900/80 rounded-2xl p-3.5 shadow-xs border border-slate-200/80 hover:opacity-100 cursor-pointer">
                  <div className="space-y-1.5">
                    <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2.5 py-0.5 rounded-full">{t.course}</span>
                    <h3 className="text-xs sm:text-sm font-bold text-slate-700 leading-snug line-clamp-1">{t.title}</h3>
                    <div className="flex items-center text-[11px] text-slate-500 pt-1.5"><Clock className="w-3 h-3 mr-1" /><span>{formattedDate} WIB</span></div>
                  </div>
                </div>
              );
            })}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-white dark:bg-zinc-900 border dark:border-zinc-800 text-slate-800 dark:text-zinc-100 rounded-3xl max-w-xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="px-6 py-5 border-b dark:border-zinc-800 flex justify-between items-start gap-4">
              <div className="space-y-1.5">
                <span className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full">{selectedDetailTask.course}</span>
                <h2 className="text-lg font-bold pt-1">{selectedDetailTask.title}</h2>
              </div>
              <button onClick={() => setSelectedDetailTask(null)} className="p-2 rounded-2xl hover:bg-slate-100 dark:hover:bg-zinc-800"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
               <div className="grid grid-cols-2 gap-3 bg-slate-50 dark:bg-zinc-800/60 p-4 rounded-2xl text-xs border border-slate-100 dark:border-zinc-800">
                <div><span className="text-slate-400 block">Dosen:</span><span className="font-bold">{selectedDetailTask.assigner}</span></div>
                <div><span className="text-slate-400 block">Prioritas:</span><span className="font-bold">{selectedDetailTask.priority}</span></div>
                <div className="col-span-2 pt-2"><span className="text-slate-400 block">Tenggat:</span><span className="font-bold text-blue-600 text-sm">{formatDeadlineDetails(selectedDetailTask.deadline)}</span></div>
              </div>
              <div className="space-y-2">
                <h4 className="text-xs font-bold uppercase">Rincian Tugas</h4>
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/80 text-xs whitespace-pre-wrap">{selectedDetailTask.description || 'Tidak ada instruksi.'}</div>
              </div>
              {selectedDetailTask.attachment && (
                <div className="space-y-2">
                  <h4 className="text-xs font-bold uppercase">Lampiran File</h4>
                  <a href={selectedDetailTask.attachment.fileUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-3.5 rounded-2xl bg-slate-50 hover:bg-slate-100 border text-xs font-semibold">
            ...
            <div className="px-6 py-4 border-t dark:border-zinc-800 flex justify-between bg-white dark:bg-zinc-900">
              {isOfficer ? (
                <div className="flex gap-2">
                  <button onClick={() => { const t = selectedDetailTask; setSelectedDetailTask(null); handleOpenEditModal(t); }} className="px-3.5 py-2 bg-slate-100 rounded-2xl text-xs font-semibold flex items-center gap-1.5"><Edit2 className="w-3.5 h-3.5" />Edit</button>
                  <button onClick={() => { onDeleteTask(selectedDetailTask.id); setSelectedDetailTask(null); }} className="px-3.5 py-2 bg-red-50 text-red-600 rounded-2xl text-xs font-semibold flex items-center gap-1.5"><Trash2 className="w-3.5 h-3.5" />Hapus</button>
                </div>
              ) : <div/>}
              <button onClick={() => setSelectedDetailTask(null)} className="px-6 py-2 bg-slate-100 rounded-2xl font-bold text-xs">Tutup</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border dark:border-zinc-800 text-slate-800 dark:text-zinc-100 rounded-3xl max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between shrink-0">
              <h3 className="text-lg font-bold">{editingTaskId ? 'Edit Tugas' : 'Tambah Tugas Baru'}</h3>
              <button type="button" onClick={() => setShowModal(false)} className="p-2 rounded-2xl text-slate-400 hover:bg-slate-100"><X className="w-5 h-5" /></button>
            </div>
            ...
            <form onSubmit={handleTaskFormSubmit} className="flex flex-col flex-1 overflow-hidden">
              <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5">Mata Kuliah</label>
                    <input type="text" required value={course} onChange={(e) => setCourse(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-zinc-800 border dark:border-zinc-700 text-xs focus:ring-2 focus:ring-blue-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5">Jenis Tugas</label>
                    <select value={type} onChange={(e) => setType(e.target.value as any)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-zinc-800 border dark:border-zinc-700 text-xs outline-none focus:ring-2 focus:ring-blue-500"><option value="Individu">Individu</option><option value="Kelompok">Kelompok</option></select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5">Tanggal Deadline</label>
                    <input type="date" required value={deadlineDate} onChange={(e) => setDeadlineDate(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-zinc-800 border dark:border-zinc-700 text-xs outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5">Jam Deadline</label>
                    <input type="time" value={deadlineTime} onChange={(e) => setDeadlineTime(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-zinc-800 border dark:border-zinc-700 text-xs outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-semibold mb-1.5">Instruksi Tugas</label>
                  <textarea rows={2} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-zinc-800 border dark:border-zinc-700 text-xs outline-none focus:ring-2 focus:ring-blue-500" />
                </div>

                {/* Drag and Drop Zone */}
                <div>
                  <label className="block text-xs font-semibold mb-1.5">Lampiran File (Opsional)</label>
                  <div
                    ...
                    className={`relative border-2 border-dashed rounded-3xl p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-all ${
                      isDragOver ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800 hover:bg-slate-100 dark:hover:bg-zinc-700/50'
                    }`}
                  >
                    ...
                    {selectedFile ? (
                      <div className="flex flex-col items-center gap-2">
                        <div className="p-3 bg-blue-100 dark:bg-blue-900/40 rounded-full"><FileIcon className="w-6 h-6 text-blue-600 dark:text-blue-400" /></div>
                        <p className="text-xs font-bold text-slate-700 dark:text-zinc-300 max-w-[200px] truncate">{selectedFile.name}</p>
                        <p className="text-[10px] text-slate-500">{(selectedFile.size / 1024 / 1024).toFixed(2)} MB • Siap diunggah</p>
                      </div>
                    ) : existingAttachment ? (
                      <div className="flex flex-col items-center gap-2">
                        <div className="p-3 bg-slate-200 dark:bg-zinc-700 rounded-full"><Paperclip className="w-6 h-6 text-slate-600 dark:text-zinc-400" /></div>
                        <p className="text-xs font-bold text-slate-700 dark:text-zinc-300 max-w-[200px] truncate">{existingAttachment.fileName}</p>
                        <p className="text-[10px] text-slate-500">File sudah tersimpan sebelumnya. Klik untuk mengganti.</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-2">
                         <div className="p-3 bg-slate-200 dark:bg-zinc-700 rounded-full"><UploadCloud className="w-6 h-6 text-slate-600 dark:text-zinc-400" /></div>
                        <p className="text-xs font-bold text-slate-700 dark:text-zinc-300">Klik atau seret file ke sini</p>
                        <p className="text-[10px] text-slate-500">PDF, Word, Excel, Gambar (Maks 10MB)</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="px-6 py-4 border-t dark:border-zinc-800 flex justify-end gap-3 bg-white dark:bg-zinc-900">
                <button type="button" disabled={isUploading} onClick={() => setShowModal(false)} className="px-5 py-2.5 rounded-2xl bg-slate-100 font-semibold text-xs disabled:opacity-50">Batal</button>
                <button type="submit" disabled={isUploading} className="px-5 py-2.5 rounded-2xl bg-blue-600 text-white font-semibold text-xs flex items-center gap-2 disabled:opacity-70">
                  {isUploading ? <><Loader2 className="w-4 h-4 animate-spin" /> Mengunggah...</> : 'Simpan Tugas'}
                </button>
              </div>
            </form>
          </div>
