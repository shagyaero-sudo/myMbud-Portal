...
export default function LayeredWave({
  ...
  /* 
    SANGAT SMART: Menggunakan CSS Variables langsung dari index.css kamu!
    - wave-bg        -> Mengikuti background dasar darkmode (#0e0f12) atau blue-950
    - wave-secondary -> Mengikuti --glow-1 (Warna Aksen Utama)
    - wave-accent    -> Mengikuti --glow-2 (Warna Glow/Sorot Terang)
  */
  const vBg = "var(--color-blue-950, #0e0f12)";
  ...
  return (
    <div className={`relative isolate overflow-hidden ${className}`}>
      <svg
        aria-hidden="true"
        focusable="false"
        className="pointer-events-none absolute inset-0 -z-10 block h-full w-full transition-all duration-500"
