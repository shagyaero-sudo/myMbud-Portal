  ...
  return (
    <div className="w-full mx-auto flex justify-center select-none">
      <div
        ...
        className="w-full aspect-square bg-white dark:bg-zinc-900 p-3 sm:p-4 rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-lg relative flex flex-col justify-between"
      >
        ...
        <AnimatePresence>
          {floatingTexts.map((ft) => (
            <motion.div
              ...
              style={{ top: `${ft.y}%` }}
            >
              <span className="text-2xl sm:text-3xl font-black text-amber-300 drop-shadow-[0_4px_12px_rgba(245,158,11,0.8)] tracking-tight">
                {ft.text}
              </span>
              {ft.subtext && (
                <span className="text-sm font-extrabold text-cyan-300 drop-shadow-[0_2px_8px_rgba(6,182,212,0.8)] uppercase tracking-wider">
                  {ft.subtext}
                </span>
              )}
            </motion.div>
        ...
        <div
          ref={gridRef}
          className="p-2 bg-slate-100 dark:bg-zinc-950 rounded-2xl border border-slate-200 dark:border-zinc-800/80 w-full h-full grid grid-cols-8 gap-1.5 sm:gap-2 touch-none"
        >
            ...
            rowArr.map((cellColor, cIdx) => {
              ...
              return (
                <div
                  ...
                  className="w-full h-full aspect-square rounded-sm sm:rounded-md relative cursor-pointer overflow-hidden transition-all duration-100 flex items-center justify-center bg-zinc-900 border border-zinc-800/40 hover:bg-zinc-800/40"
                >
                  ...
                  {cellColor && !isClearing && (
                    <motion.div
                      ...
                      className={`w-full h-full rounded-sm sm:rounded-md bg-gradient-to-b ${colorInfo?.gradient} border-t border-white/20 shadow-[inset_0_0_10px_rgba(0,0,0,0.35)] shadow-md relative overflow-hidden`}
                    >
                      {/* Top Gloss Shine */}
                      <div className="absolute top-0 left-0 right-0 h-[38%] bg-white/20 rounded-t-xs"></div>
                      {/* Inner Bottom Shadow */}
                      <div className="absolute bottom-0 left-0 right-0 h-[25%] bg-black/30 rounded-b-xs"></div>
                    </motion.div>
                  )}
                  ...
                  {isGhost && !cellColor && (
                    <div
                      className={`w-full h-full rounded-sm sm:rounded-md border-2 z-10 transition-colors ${
                        isValid
                          ? 'bg-emerald-400/40 border-emerald-300 animate-pulse'
                          : 'bg-rose-500/25 border-rose-400/80'
                      }`}
                    />
                  )}
                </div>
